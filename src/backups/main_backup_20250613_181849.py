...
class PagedTableLoader(QThread):
    ...
    def __init__(self, engine, model_class, filter_text="", offset=0, limit=50):
        super().__init__()
        self.engine = engine
...
class MainWindow(QMainWindow):
    ...
    def create_ingredients_page(self):
        ...
        self.ingredient_table.setSortingEnabled(True)  # 启用排序
    ...
    def create_formulas_page(self):
        ...
        self.formula_table.setSortingEnabled(True)  # 启用排序
    ...
    def refresh_ingredient_table(self):
        ...
        # Create new loader with enhanced filter logic
    ...
    def refresh_formula_table(self):
        ...
        # Create new loader
        self.formula_loader = PagedTableLoader(self.engine, Formula, keyword, offset, self.formula_page_size)
...
class StockRecordDialog(QDialog):
    ...
    def __init__(self, parent, session):
        ...
        # 原料选择区域
        ingredient_group = QGroupBox("🧪 原料选择")
        ...
        self.ingredient_search.textChanged.connect(self.filter_ingredients)
        search_layout.addWidget(self.ingredient_search)
        ingredient_layout.addLayout(search_layout)
        
        # 原料下拉框
        self.ingredient_combo = QComboBox()
        self.ingredient_combo.setEditable(True)  # 允许编辑以支持搜索
        ...
        main_layout.addWidget(ingredient_group)
        
        # 库存信息区域
        stock_group = QGroupBox("📦 库存信息")
        form = QFormLayout()
        self.quantity_edit = QDoubleSpinBox()
        self.quantity_edit.setRange(-999999, 999999)
        self.quantity_edit.setDecimals(4)
        ...
        form.addRow("有效期", self.expiration_edit)
        
        stock_group.setLayout(form)
        main_layout.addWidget(stock_group)
    ...
    def populate_ingredient_combo(self, filter_text=""):
        ...
        for ing in self.all_ingredients:
            ...
            if (not filter_text or 
                filter_text in ing.name.lower() or 
                filter_text in (ing.number or "").lower() or 
                filter_text in (ing.cas_number or "").lower()):
        ...
        for ing in filtered_ingredients:
            display = f"{ing.number or 'N/A'} - {ing.name}"
            if ing.cas_number:
                display += f" (CAS:{ing.cas_number})"
            self.ingredient_combo.addItem(display)
            self.ingredient_map[display] = ing
    
    def filter_ingredients(self):
        """根据搜索文本筛选原料"""
        search_text = self.ingredient_search.text().strip()
        current_text = self.ingredient_combo.currentText()
        
        self.populate_ingredient_combo(search_text)
        
        # 尝试保持当前选择
        if current_text and current_text in self.ingredient_map:
            self.ingredient_combo.setCurrentText(current_text)
    
    def update_ingredient_info(self):
        """更新原料信息显示"""
        current_text = self.ingredient_combo.currentText()
        if current_text and current_text in self.ingredient_map:
            ing = self.ingredient_map[current_text]
            
            # 获取当前库存
            from sqlalchemy import func
            current_stock = self.session.query(func.sum(StockRecord.quantity)).filter_by(ingredient_id=ing.id).scalar() or 0
            
            info_text = f"""
📋 原料信息：
• 名称：{ing.name}
• 编号：{ing.number or 'N/A'}
• CAS号：{ing.cas_number or 'N/A'}
• 当前库存：{current_stock:.4f}g
• 英文名：{ing.english_name or 'N/A'}
            """.strip()
            
            self.ingredient_info_label.setText(info_text)
            self.ingredient_info_label.setStyleSheet("""
                QLabel {
                    background-color: #e8f5e8;
                    border: 1px solid #4caf50;
    ...
    def get_data(self):
        ...
        ing = self.ingredient_map[ing_display]
